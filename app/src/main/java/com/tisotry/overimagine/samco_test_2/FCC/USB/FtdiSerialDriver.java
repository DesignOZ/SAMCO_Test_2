...
package com.tisotry.overimagine.samco_test_2.FCC.USB;
        ...
        public int read(byte[] dest, int timeoutMillis) throws IOException {
            ...
            if (ENABLE_ASYNC_READS) {
                ...
                synchronized (mReadBufferLock) {
                    ...
                    // 페이로드만이 아닌 전부 다 보내는 식으로 츄라이
                    // FCC수신기로부터 아무런 정보가없어도 GCS수신기가 자신의 상태를 계속 앱으로 보내게 되있음
                    // 계속 GCS수신기로 부터 2바이트 정보를 받게됨. 모뎀의 상태 헤더 정보인듯 함.
                    //System.arraycopy(mReadBuffer, 0, dest, 0, readAmt);
                    //return totalBytesRead;

                    //filterStatusBytes(mReadBuffer, dest, totalBytesRead, endpoint.getMaxPacketSize());
/*                    mReadQueue.put(mReadBuffer);
                    if(mReadQueue.get() == 0x44) {
                        while (mReadQueue.get() != 0x44) {
        ...
        public byte[] syncStream(byte[] receivedData) {
            ...
            int length = (i2-2)-i+1;
            byte[] checklast = new byte[length];
            buf.get(checklast, i, length);
            //updateReceivedData(checklast);
            return checklast;
        }
